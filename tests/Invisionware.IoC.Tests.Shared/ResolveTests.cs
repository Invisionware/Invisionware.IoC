...
using XLabs.IOC.NUnit.Shared.Services;
#if WINDOWS_PHONE
...
namespace IocTests
{
	...
	public abstract class ResolveTests
	{
		...
		public void CanCreateWithConstructorParameter()
		{
			...
			container.Register<IMyService, MyServiceWithConstructorParam>();

			var resolver = container.GetResolver();

			var service = resolver.Resolve<IMyService>(new { age = 10 });

			Assert.IsTrue(service is MyServiceWithConstructorParam);

			if (resolver.IsConstructorArgsSupported)
			{
				Assert.AreEqual(((MyServiceWithConstructorParam)service).Age, 10);
			}
		}
